...
<template>
  <div
    class="col-span-1 rounded-lg h-[400px] w-3/5 px-3 bg-white mx-auto flex flex-col gap-y-5 justify-center items-center text-center"
  >
    <h2 class="text-xl text-black">
      <router-link :to="`/product/${props.product.id}`"> {{ props.product.name }}</router-link>
    </h2>
    ...
    <button @click="addProductToCart(props.product.id)" class="bg-black px-4 py-2">
      Add to Cart
    </button>
  </div>
</template>
